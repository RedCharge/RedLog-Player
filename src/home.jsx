...
import { 
  ...
  Volume2, Search, Home as HomeIcon, Library, Heart, 
  MoreHorizontal, Plus, Clock, Layers, Menu, X,
...
const Home = () => {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#050505] text-white overflow-hidden flex flex-col lg:flex-row font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 h-screen p-6 flex-col bg-black/40 backdrop-blur-3xl border-r border-white/5 transition-transform duration-300 lg:relative lg:flex ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="flex items-center gap-3 mb-10 px-2">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden ml-auto p-2 text-white/60 hover:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
        <nav className="flex flex-col gap-2">
          <SidebarLink icon={<HomeIcon />} label="Discover" active={activeTab === 'home'} onClick={() => setActiveTab('home')} />
      ...
      <main className="flex-1 h-screen overflow-y-auto custom-scrollbar z-10">
        <header className="sticky top-0 z-40 flex items-center justify-between px-6 lg:px-12 py-6 bg-gradient-to-b from-black/80 to-transparent">
          ...
          <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 bg-white/5 rounded-xl">
